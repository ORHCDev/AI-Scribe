def deidentifier_model(file_seed, device, num_workers, batch_size, hospitals, vendors):
    ...
    """
    # If above isn't working, download the StanfordAIMI/stanford-deidentifier-base from hugging face
    # Adjust the model and tokenizer paths according nto where you place the model
    classifier = pipeline(
        "token-classification",
        model="./Stanford_Penn_Deidentifier/stanford-deidentifier-base",
        tokenizer="./Stanford_Penn_Deidentifier/stanford-deidentifier-base",
        device=device,
        num_workers=num_workers,
        batch_size=batch_size,
    )
