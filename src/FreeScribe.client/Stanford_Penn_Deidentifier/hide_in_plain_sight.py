def hide_in_plain_sight(file_seed):
    ...
    # File locations
    SSA_NAMES = "./Stanford_Penn_Deidentifier/SSA_Names_DB.xlsx"
    SURNAMES = "././Stanford_Penn_Deidentifier/Common_Surnames_Census_2000.csv"
    HOSPITALS = "./Stanford_Penn_Deidentifier/hospitals.txt"
    FREQ_HOSPITALS = "./Stanford_Penn_Deidentifier/frequent_hospitals_tokens.txt"
    UNIVERSITIES = "./Stanford_Penn_Deidentifier/universities.txt"
    COMPANIES = "./Stanford_Penn_Deidentifier/companies.txt"
    ...
    def get_min_max_dates(date_list, convert_string_to_date):
        ...
        for date in date_list:
            ...
            index += 1
        if min_date == None:
            min_date = datetime.datetime.strptime("1/1/2000", "%m/%d/%Y")
    ...
    # Database surnames
    data_surnames = (
        pd.DataFrame(pd.read_csv(SURNAMES).iloc[:, 0])
        .astype("str")
        .drop_duplicates()
        .dropna()
    )
    ...
    # Database firstnames
    data_firstnames = (
        pd.DataFrame(pd.read_excel(SSA_NAMES).iloc[:, 0])
        .astype("str")
        .drop_duplicates()
        .dropna()
    )
    ...
    try:
        data_hospitals = (
            pd.read_csv(HOSPITALS, delimiter="\t", header=None)
    ...
    data_universities = (
        pd.read_csv(UNIVERSITIES, delimiter="\t", header=None)
    ...
    data_frequent_tokens = (
        pd.read_csv(FREQ_HOSPITALS, delimiter="\t", header=None)
